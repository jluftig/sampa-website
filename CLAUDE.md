...
17. **Board meetings** — `/board` + `/board/:slug` gated like the directory (`RequireActiveMember` / `is_active_member()`). `is_board` is still badge-only. Seed: `src/data/boardMeetings.js`; PDFs in `public/files/board/`.
