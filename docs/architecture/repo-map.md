...
| Signed-in | `/dashboard` |
| Active member or staff | `/members`, `/members/:id` (peer directory — not staff roster); `/board` (Agendas / Records / Schedule tabs), `/board/:slug` (Board meeting agenda + minutes — `RequireActiveMember`, not `is_board`) |
