...
src/
  ...
  data/
    ...
    leadership.js           Public leadership roster (form submissions; add a person = append)
  pages/                    Home, About, Leadership, News, PostView, Policy, PolicyView, Tags, TagView, Search,
...
| Audience | Paths |
|----------|--------|
| Public | `/`, `/about`, `/leadership`, `/news`, `/news/:slug` (`#point-<item id>`), `/policy`, `/policy/:slug`, `/keywords`, `/keywords/:slug` (`?and=` intersection), `/search?q=`, `/login`, `/join`, `/donate`, `/privacy`, `/terms` |
| Signed-in | `/dashboard` |
