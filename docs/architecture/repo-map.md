...
| Signed-in | `/dashboard` |
| Active member or staff | `/members`, `/members/:id` (peer directory — not staff roster); `/board`, `/board/:slug` (Board meeting schedule, agendas, approved minutes — `RequireActiveMember`, not `is_board`) |
