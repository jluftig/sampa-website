...
|----|------|-------|---------|-------|
| T24 | Employer / institutional membership invoice request | cursor | 2026-08-16 | **Claimed cursor.** Grok 4.6 cloud agent `bc-18ace466`. Academic/hospital PAs need a pre-payment invoice (Stripe /join only receipts after charge). Form + Brevo transactional email to `treasurer@` (cc `josh@`). Not Stripe Invoicing. **PR #73** (draft) — preview only; **do not merge / do not ship to production** until Josh reviews the Vercel preview. |
...
- **Employer / institutional invoice request (T24 · cursor)** — `/join` link + `/join/invoice` form so a PA can request a SAMPA invoice before paying (employer reimbursement). Staff email via existing Brevo transactional `POST /smtp/email` (From `info@`, not Josh; not a campaign). **PR #73** (draft). Preview only; do not merge until Josh reviews the Vercel preview.
