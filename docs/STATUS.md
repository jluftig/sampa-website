...
|----|------|-------|---------|-------|
| T24 | Employer / institutional membership invoice request | cursor | 2026-08-16 | **Claimed cursor.** Academic/hospital PAs need a pre-payment invoice (Stripe /join only receipts after charge). Form + Brevo transactional email to `treasurer@` (cc `josh@`). Not Stripe Invoicing. **PR #73** (draft) — preview only; **do not merge / do not ship to production** until Josh reviews the Vercel preview. |
