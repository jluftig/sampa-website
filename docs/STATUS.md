...
|----|------|-------|---------|-------|
| T56 | Homepage / `/editor/members` reload loop (Shani Wilson) | cursor | 2026-09-18 | **Claimed cursor (continued 2026-09-23).** PR #111 (preview only). PR #109 (`d749cc7`) stopped the loop for non-viewers and half-sessions. Privileged viewers still bounced when the session dropped to null, because Login honors `next=/editor/members`. Hold that null frame (`AUTH_NULL_HOLD_MS`) on guarded pages before `/login`. Cold signed-out still goes to login once. Do not merge until Website QA + Josh. |
