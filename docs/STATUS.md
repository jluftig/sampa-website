...
**Last updated:** 2026-08-16 (T24 claimed cursor: employer/institutional membership invoice request; preview PR only — do not merge)
...
|----|------|-------|---------|-------|
| T24 | Employer / institutional membership invoice request | cursor | 2026-08-16 | **Claimed cursor.** Grok 4.6 cloud agent `bc-18ace466`. Academic/hospital PAs need a pre-payment invoice (Stripe /join only receipts after charge). **`/membership` is Step 1** (benefits, eligibility, one-year vs auto-renew equally, then checkout + employer invoice). Form at `/join/invoice`; Brevo transactional to `treasurer@` (cc `josh@`). Not Stripe Invoicing. **PR #73** (draft) — preview only; **do not merge / do not ship to production** until Josh reviews the Vercel preview. |
...
- **Employer / institutional invoice request (T24 · cursor)** — public **`/membership`** (Step 1) + `/join` checkout (Step 2) + `/join/invoice` form. Nav Membership points at `/membership`, not the homepage anchor. Staff email via existing Brevo transactional `POST /smtp/email` (From `info@`, not Josh; not a campaign). **PR #73** (draft). Preview only; do not merge until Josh reviews the Vercel preview.
