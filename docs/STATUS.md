...
**Last updated:** 2026-09-14 (T50 In Progress — Member Login routing for signed-in editors/members)
...
|----|------|-------|---------|-------|
| T50 | Member Login routing for signed-in editors/members | cursor | 2026-09-14 | **Claimed cursor.** Code fix on `cursor/member-login-routing-fde8`: signed-in editors → `/editor`; members → `/dashboard`; join upsell only when *this* auth user's profile has no `membership_status`. Josh `luftig@gmail.com` screenshot (Welcome / no name / no editor link / no membership) is consistent with a second profile, not a wrong-table lookup. Do not invent a production merge — run the diagnostic SQL in the PR. |
