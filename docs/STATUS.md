...
**Last updated:** 2026-09-24 (T60: Recent issues titles truncate, and parenthetical Brevo notes are hidden on the label. Draft PR #115. Do not merge.)
...
|----|------|-------|---------|-------|
| T60 | Recent issues titles stay on one line | cursor | 2026-09-24 | **Claimed cursor.** Draft PR #115. Reach → Recent issues on `/editor/members`. The row truncates, and the label hides parenthetical Brevo notes such as `(TEST)` and `(FIRSTNAME + feedback ready)`. The raw name stays on the issue and on the tooltip. TEST sends still count and still show. Preview only. Do not merge until Website QA and Josh. |
