...
**Last updated:** 2026-09-06 (T46 In Progress — session persistence across mobile nav + Stripe return)
...
|----|------|-------|---------|-------|
| T46 | Session persistence across mobile navigation + Stripe checkout return | cursor | 2026-09-06 | **Claimed cursor.** Vic Holmes (via Shani): mobile re-login on every screen change; laptop logged out after 3-year membership checkout before a follow-on $75 donate. Harden Supabase session (retry refresh, storage backup, www canonical URLs). Preview PR only — do not merge until Josh reviews. |
