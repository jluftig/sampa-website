...
|----|------|-------|---------|-------|
| T56 | Homepage / `/editor/members` reload loop (Shani Wilson) | cursor | 2026-09-18 | **Claimed cursor.** Shani opened www.addictionpas.org (likely via Josh email to `/editor/members`) and the page kept loading/reloading. Trace RequireMemberViewer + Member Login + stale-session redirects; land on a stable page; do not weaken `canViewMemberRoster`. |
