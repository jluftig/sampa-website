...
|----|------|-------|---------|-------|
| T52 | Vercel Web Analytics for addictionpas.org | cursor | 2026-09-16 | **Claimed cursor.** Pageview tracking via `@vercel/analytics` on the Vite SPA. Preview PR only — do not merge until Josh reviews. After merge, confirm Vercel project → Analytics is Enabled (dashboard toggle is separate from the package). |
