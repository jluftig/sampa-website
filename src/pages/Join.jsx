...
import { MEMBERSHIP_TIERS, tierByKey } from '../lib/membership';
import { apiPost } from '../lib/api';
import Navbar from '../components/Navbar';
import Footer from '../components/Footer';

// Join flow, account-first: sign in (1 click) → pick a tier → Stripe Checkout.
// Checkout is created server-side with the user's id as client_reference_id,
// so the webhook can activate the right profile without email matching.
export default function Join() {
  ...
  const selectTier = async (tierKey) => {
    ...
    setBusyTier(tierKey);
    try {
      const { url } = await apiPost('/api/create-checkout-session', { tier: tierKey });
  ...
  return (
    <div className="relative min-h-screen bg-background text-text">
      ...
      <main className="max-w-6xl mx-auto px-4 pt-32 pb-24">
        ...
        {/* Multi-year note (matches the homepage) */}
        <div className="max-w-3xl mx-auto bg-primary/5 border border-primary/20 rounded-3xl p-6 mb-12 flex items-center gap-4 shadow-sm">
          ...
          <p className="text-text/80 text-sm">
            Memberships renew annually and are easy to cancel. Multi-year discount
            pricing is coming to online checkout — contact us if you'd like a 2- or
            3-year plan today.
          </p>
        </div>
            ...
            >
              ...
              <div className="relative z-10">
                ...
                <div className={`text-4xl font-bold font-sans mb-8 ${tier.highlight ? 'text-white' : 'text-primary'}`}>
                  {tier.price}
                  <span className={`text-lg font-normal ${tier.highlight ? 'text-white/50' : 'text-text/50'}`}>/yr</span>
                </div>
              </div>
              <button
